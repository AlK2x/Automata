...
namespace
{
	...
	Value *GenerateUnaryExpr(IRBuilder<> & builder, LLVMContext &context, UnaryOperator op, Value *x)
	{
		(void)context;
		switch (op)
		{
	...
	// Number -> double
	// String -> i8*
	Type *ConvertType(LLVMContext &context, BaseType type)
	{
		switch (type)
		{
...
void CFunctionCodeGenerator::Codegen(const ParameterDeclList &parameters, const StatementList &block, Function &fn)
{
...
{
	BaseType type = ast.GetValue().GetType();
...
void CFunctionCodeGenerator::Visit(CAssignmentAST &ast)
{
...
void CFunctionCodeGenerator::CodegenLoop(CWhileAST &ast, bool skipFirstCheck)
{
	auto & context = m_context.GetLLVMContext();
	Function *function = m_builder.GetInsertBlock()->getParent();
...
void CFunctionCodeGenerator::FillBlockAndJump(const StatementList &statements, BasicBlock *block, BasicBlock *nextBlock)
{
...
Function *CCodeGenerator::AcceptFunction(IDeclarationAST &ast)
{
...
Function *CCodeGenerator::AcceptMainFunction(IDeclarationAST &ast)
{
...
Function *CCodeGenerator::GenerateDeclaration(IDeclarationAST &ast, bool isMain)
{
	auto & context = m_context.GetLLVMContext();
...
bool CCodeGenerator::GenerateDefinition(Function &fn, IDeclarationAST &ast, bool isMain)
{
